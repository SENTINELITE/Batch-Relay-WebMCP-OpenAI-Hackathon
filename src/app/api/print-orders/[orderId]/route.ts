...
import { errorResponse, forwardUpstream, hasStudioTokenConfiguration, opaquePathSegment } from "@/lib/batch-relay/server";
...
export async function GET(_: Request, { params }: { params: Promise<{ orderId: string }> }) {
  return route(async () => {
    ...
    return orderID?.startsWith("bro_")
      ? forwardUpstream(`/v1/print-orders/${orderID}`, { authorization: hasStudioTokenConfiguration() ? "studio" : "anonymous" })
