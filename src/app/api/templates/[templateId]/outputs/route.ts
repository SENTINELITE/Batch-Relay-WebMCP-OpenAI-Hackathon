...
import { frozenDemoRevisionMatches, frozenDemoTemplate, frozenDemoTemplateOutputs } from "@/lib/storefront/template-specs/frozen-demo";
...
export async function GET(request: Request, { params }: { params: Promise<{ templateId: string }> }) {
  return route(async () => {
    ...
    if (templateID !== frozenDemoTemplate.templateID) return errorResponse(404, "frozen_demo_template_not_found", "This template is not part of the frozen demo.");
